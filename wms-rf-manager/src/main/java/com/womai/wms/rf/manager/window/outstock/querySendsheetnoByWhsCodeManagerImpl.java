...
import com.womai.wms.rf.domain.outstock.QuerySendsheetno;
 ...
 */
@Scope("prototype")
@Component("querySendsheetnoByWhsCodeManager")
public class querySendsheetnoByWhsCodeManagerImpl extends ReceiveManager {
    ...
    private static final String[] pageHeader = {Constants.BREAK_LINE, TipConstants.OUTSTOCK_PICKUP_SENDSHEETNO, Constants.SPLIT, ""};
    @Autowired
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        this.ctx = ctx;
        //进入集周转箱页面
        super.initBaseMap(QuerySendsheetno.class, pageHeader, ctx);
    }
    /**
     * 接收用户输入
     *
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        QuerySendsheetno querySendsheetno = (QuerySendsheetno) accepterMap.get(DefaultKey.objectClass.keyName);
        ...
        if (WAREHOUSE_CODE.equals(lastCompleteColName)) {
            String realstorewhscode = querySendsheetno.getRealstorewhscode();
                RemoteResult<String> remoteResult = outstockPickupRemoteService.querySendsheetnoByWhsCode(getCredentialsVO(ctx), realstorewhscode);
                if (remoteResult.isSuccess()) {
                    if(StringUtils.isNotBlank(remoteResult.getT())){
                        HandlerUtil.clearAll(ctx.channel());
                        HandlerUtil.writer(ctx, pageHeader, 1, 1);
                        HandlerUtil.println(ctx, Constants.RF_MANAGER_SENDSHEETNO + remoteResult.getT());
